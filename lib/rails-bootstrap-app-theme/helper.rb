module RailsBootstrapAppTheme
  module Helper
    ...
    def navbar_brand(&block)
      yield links = NavigationBuilder.new if block_given?

      links.collect do |link|
        link_to(link[:label], link[:href], class: link[:class] << ' brand')
      end.join("").html_safe
    end
    ...
    def sidebar_navigation(options={}, &block)
      options[:class] ||= " sidebar-nav"
      options[:class].strip!

      box_buffer = NavigationBuilder.new(self)
      box_content = capture(box_buffer, &block)

      content_tag("div", "", options) do
        block_out = content_tag("ul", "", class: "nav nav-list") do
          box_buffer.collect { |item|
            content_tag("li", class: item[:class]) do
              if item[:href]
                link_to(item[:label], item[:href], item[:link_options])
              else
        ...
        items.collect { |item|
          content_tag("li", class: item[:class]) do
            if item[:active]
              ...
              link_to(item[:label], item[:href]) + raw('<span class="divider">/</span>')
            end
          end
        }.join("").html_safe
      end
